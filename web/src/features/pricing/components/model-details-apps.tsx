...
function getRankPalette(rank: number): string {
  if (rank === 1) {
...
function getGrowthPalette(isUp: boolean, isDown: boolean): string {
  if (isUp) {
...
function getGrowthIcon(isUp: boolean, isDown: boolean) {
  if (isUp) return ArrowUpRight
  if (isDown) return ArrowDownRight
  return null
}
...
function GrowthChip(props: { value: number }) {
  ...
  const palette = getGrowthPalette(isUp, isDown)
  const Icon = getGrowthIcon(isUp, isDown)
