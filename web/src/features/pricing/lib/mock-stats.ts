...
type RateLimitBase = { baseRpm: number; baseTpm: number; baseRpd: number }

function rateLimitBaseFor(isHeavy: boolean, isLight: boolean): RateLimitBase {
  if (isHeavy) return { baseRpm: 60, baseTpm: 0, baseRpd: 1_000 }
  if (isLight) return { baseRpm: 5_000, baseTpm: 1_000_000, baseRpd: 100_000 }
  return { baseRpm: 500, baseTpm: 200_000, baseRpd: 10_000 }
}
...
export function buildRateLimits(model: PricingModel): RateLimit[] {
  ...
  const { baseRpm, baseTpm, baseRpd } = rateLimitBaseFor(isHeavy, isLight)
